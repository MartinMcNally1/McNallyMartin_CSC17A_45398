...
struct Plr{     //Struct info for player
    ...
    bool stay;      //If flag is true, player has chosen to stay
    bool cashout;   //Determines if a player has cashed out or is still betting
...
struct House{       //Struct Info for dealer
    string hand;
    int handV;  
    bool ace;       //Flag is true if there is an ACE in dealer deck
...
{    
    ...
    int table;      //Table number (row)
    int floor;      //Floor number of table (colomn)
    bool end;       //Once all players have cashed out, flag is true.   
...
//Function Prototypes
Info *GetGame();     //Dynamically creates the number of players
    //Player Fucntions
...
void AceChkP( Info*,int i );     //If the player bust but has a 21 card, minus 10
...
void Reset(Info*, int i);     //Resets flags and clear hand for next round of betting for both dealer and player
...
int main(int argc, char** argv) {
    ...
    Game=GetGame(); //Get Table number, floor, and the number of players and balance of each player
...
Info *GetGame(){  //Creates the Struct that Will be used for the whole game(Players involved)
    
    Info *tempH;    //Temp strcut pointer
    ...
    tempH->player = new Plr[tempH->size]; //Allocates Dynamic Memory for struct array of players
    
    cout << "Enter balance of each player at the table." << endl;
    for ( int i=0;i<tempH->size;i++){
        cout << "Player " << i+1 << ": $";
        cin >> tempH->player[i].balance;
        while (tempH->player[i].balance < 10){
...
void Delete(Info *game){  //Deallocate dynamic struct 
...
void GetBet(Info *game, int i){
    if(game->player[i].balance==0){
        cout << "Sorry, you have no more money to spend." << endl;
        cout << "You are forced to cash out";
        game->player[i].cashout=true;
    }else{
        cout << "Starting bet for player " << i << " : $";
    ...
    do{
        ...
        //These checks must always be passed through
            ...
            if(game->player[i].nat21==true){    //Check if player hand is 21
                ...
                cout << "Player wins $" << game->player[i].plrBet;
                ...
                cout << "Players Hand: " << game->player[i].hand << "(value = " << game->player[i].handV << ")" << endl;
...
void Compare(Info *game, int i){ //Compare with dealers hand, see if they win or loose against dealer
    ...
    if (game->dealer.handV > game->player[i].handV){
        ...
        cout << "Player loses $" << game->player[i].plrBet;
...
void Cashout(Info *game, int i){     //If player chooses to cash out or bet again
    ...
    if( game->player[i].balance==0){
        ...
        if (num==2){
            cout << "Player " << i+1 << " has cashed out with $" << game->player[i].balance;
            game->player[i].cashout=true;
        }else{
