 ...
 *Purpose:  Create a black-jack game using structures and concepts from chapters 9-12
 * Final Verison
 *        
 * 
 */         
...
void Delete(Info*);          //Delete Allocated Memory
void SaveData(Info*,Data);   //Open, save data into a binary file, close file
...
void GetBet(Info *game, int i){
    if(game->player[i].balance==0){
        ...
        game->player[i].cashout=true;   //Cashes out player if they run out of money
    }else{
