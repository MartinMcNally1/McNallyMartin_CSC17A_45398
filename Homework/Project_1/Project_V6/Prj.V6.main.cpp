 ...
 * Ver. 5:  - Include Binary File Save
 *        
 * 
 */         
...
#include <fstream>    //Input/Output from and to files
...
struct Plr{     //Struct info for player
    int balance=0;    //Balance that player bets with
...
struct House{       //Struct Info for dealer
    ...
    int balance=0;    //How much the dealer has won from players
};
...
int main(int argc, char** argv) {
    ...
    Data data;      //struct variable for the very end
    Info *Game;     //Struct variable for all info on game
    //fstream saveF;  //File Stream, to create files, write data, and read data
    ...
    //Transfer Game Data to struct file, and save it
    SaveData(Game,data);
        //Test if transfer is good.
    //cout << Game->dealer.balance << endl;
    data.Balance=Game->dealer.balance;
//    cout << data.Balance << endl;
    data.Floor=Game->floor;
//    cout << data.Floor << endl;
...
//    cout << data.Size << endl;
    ...
    //Deallocate memory
    Delete(Game);
    //delete Game;  //Placing he delete pointer ouside the Delete function call causes everything to crash fo some reason
...
void Delete(Info *data){  //Deallocate dynamic struct memory 
    //cout << "delete call works" <<endl;;
    delete[] data->player;  //Deallocate pointer to pointer array
    cout << "delete pointer member array works" << endl;
    //delete data;          //Deallocate pointer struct
    //cout << "delete struct pointer" << endl;
    //I cannot include the delete data part or everything will fail at the end, for some reason
    cout << "Memory Deallocated" <<endl;
}

void SaveData(Info *game, Data data){
    //Transfer data from pointer struct to struct variable
    data.Balance = game->dealer.balance;
//    cout << data.Balance << endl;
    data.Floor = game->floor;
//    cout << data.Floor << endl;
...
//    cout << data.Size << endl;
    ...
    //Create File, open it 
    fstream saveF("Casino.Data", ios::out | ios::binary);    //Creates file, opens it read in and binary mode
    
    //Read in data
    saveF.write(reinterpret_cast<char *>(&data),sizeof(data));   //Reads in data to binary file
...
void plrHit(Info *game, int i){   //Adds another card to the user hand, and adds value to handV to check is bust
    ...
    if (num==TEN){
        ...
        game->player[i].ace=true;  //If true, player hand holds an Ace
    }
    else{
