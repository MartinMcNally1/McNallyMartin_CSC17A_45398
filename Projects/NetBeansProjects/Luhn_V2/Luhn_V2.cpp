...
char Luhn(char[],int );         //Perform the Luhn algorithm
...
int main(){
    ...
    //(0)American Express cards begin with 34 or 37, with 15 digits total
    //(1)Visa cards have between 13 digits total
...
void genCC ( char CC[]){
    ...
    //cout << CC << endl; //Base card
    CC[maxdigits-1]= Luhn(CC, maxdigits-1);   //Generates the Checking number and places it on the end
    //cout << CC << endl; //card with checking number
    //flipDig(CC);
    //cout << CC << endl; //Card with random digit flipped
